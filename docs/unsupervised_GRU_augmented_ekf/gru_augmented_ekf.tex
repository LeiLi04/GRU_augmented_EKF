...
\section{Experiments}
% =====================================================================
This section specifies the Exercise~5.5 scenario, the standard range observation convention, and the evaluation protocol.
...
\subsection{Multi-Anchor Range-Only Measurements (Standard Form)}
Let $M$ anchors have known fixed 2D positions
$\bm{s}^{(i)}=[s_x^{(i)},\,s_y^{(i)}]\T$, $i\in\{1,\dots,M\}$.
At time $k$, anchor $i$ produces one scalar range measurement in the standard ``target minus anchor'' form:
\begin{equation}
\theta_k^{(i)} =
\sqrt{\big(x_k - s_x^{(i)}\big)^2 + \big(y_k - s_y^{(i)}\big)^2}
+ r_k^{(i)},
\qquad
r_k^{(i)}\sim\mathcal{N}(0,\sigma_{r,i}^2).
\label{eq:range_standard}
\end{equation}
...
\subsection{EKF Measurement Jacobian for the Standard Range Form}
For anchor $i$, define
\[
\begin{aligned}
\Delta x_k^{(i)} &\triangleq \hat{x}_{k|k-1} - s_x^{(i)},\qquad
\Delta y_k^{(i)} \triangleq \hat{y}_{k|k-1} - s_y^{(i)},\\
...
Let $\bm{p}(t)=[x(t),\,y(t)]\T$ and $\bm{v}(t)=[\dot{x}(t),\,\dot{y}(t)]\T$.
We use a fixed sampling period $\Delta t = 0.01$~s throughout both data generation and filtering.

Outside maneuver segments, ground-truth trajectories are generated from the continuous-time
white-noise-acceleration (WNA) model
\begin{equation}
\dot{\bm{p}}(t)=\bm{v}(t),\qquad
\dot{\bm{v}}(t)=\bm{w}(t),
\label{eq:ct_wna}
\end{equation}
where $\bm{w}(t)$ is zero-mean white Gaussian noise with spectral density
$\bm{Q}_c=\diag(q_1^c,\,q_2^c)$.
Sampling \eqref{eq:ct_wna} with period $\Delta t$ yields the discrete-time constant-velocity model
\eqref{eq:cv} with $\bm{F}$ in \eqref{eq:F_cv} and $\bm{Q}$ in \eqref{eq:Q_wiener_velocity}.

To induce a controlled model mismatch while keeping the filter nominal constant-velocity,
we inject turn segments in the \emph{true} dynamics via a constant turn-rate velocity model
\begin{equation}
\dot{\bm{v}}(t)=\bm{\Omega}(\omega)\bm{v}(t),\qquad
\bm{\Omega}(\omega)=
\begin{bmatrix}
0 & -\omega\\
\omega & 0
\end{bmatrix},
\label{eq:ct_turnrate}
\end{equation}
with $\dot{\bm{p}}(t)=\bm{v}(t)$ for position integration.
The exact discretization of \eqref{eq:ct_turnrate} over $\Delta t$ gives
\begin{equation}
\bm{v}_{k+1}=
\exp\!\big(\bm{\Omega}(\omega)\Delta t\big)\bm{v}_{k}
=
\begin{bmatrix}
...
We use three maneuver windows (each of length $L$ discrete steps) and choose a single constant
turn rate $\omega$ such that the velocity direction rotates by $90^\circ$ over each window:
\begin{equation}
\omega=\frac{\pi/2}{L\,\Delta t}.
\label{eq:omega_90deg}
\end{equation}
In our implementation, each window contains $L=50$ update steps (left-closed, right-open),
and $\Delta t=0.01$~s, hence
\begin{equation}
\omega=\frac{\pi/2}{50\cdot 0.01}=\pi \ \text{rad/s}.
\label{eq:omega_numeric}
\end{equation}
Across three such windows, the ground-truth velocity accumulates a total rotation of $270^\circ$.
During these windows, the filter still assumes the nominal CV transition \eqref{eq:cv}
(i.e., $\omega=0$), which creates the intended motion-model mismatch.

We evaluate:
\begin{itemize}
\item \textbf{Matched:} no turn segments (equivalently $\omega=0$ for all $k$).
\item \textbf{Mismatch:} three $90^\circ$ turn segments with $\omega=\pi$~rad/s in the ground truth, while the filter assumes $\omega=0$.
\end{itemize}
...
\begin{itemize}
\item \textbf{EKF (nominal):} EKF with nominal $(\bm{F},\bm{Q}_0)$ and range model \eqref{eq:range_standard}.
